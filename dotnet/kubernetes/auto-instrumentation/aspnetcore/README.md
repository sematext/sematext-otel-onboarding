# .NET ASP.NET Core - Auto-Instrumentation (Kubernetes)

This example demonstrates automatic OpenTelemetry instrumentation for a .NET ASP.NET Core application deployed to Kubernetes.
...
helm install sematext-agent sematext/sematext-agent \
  ...
  --set otel.services.dotnet-aspnetcore-k8s-auto=dotnet-group \
...
```bash
docker build -t dotnet-aspnetcore-k8s-auto:latest .
```

### 4. Load Image into Minikube

```bash
minikube image load dotnet-aspnetcore-k8s-auto:latest
```
...
```bash
# Check pod status
kubectl get pods -l app=dotnet-aspnetcore-k8s-auto

# Check logs
kubectl logs -l app=dotnet-aspnetcore-k8s-auto -f
```
...
kubectl port-forward svc/dotnet-aspnetcore-k8s-auto 8080:80
...
kind: Deployment
metadata:
  name: dotnet-aspnetcore-k8s-auto
spec:
  replicas: 1
  template:
    spec:
      containers:
      - name: dotnet-aspnetcore-k8s-auto
        image: dotnet-aspnetcore-k8s-auto:latest
        env:
        - name: OTEL_SERVICE_NAME
          value: "dotnet-aspnetcore-k8s-auto"
...
This example uses the [OpenTelemetry .NET Automatic Instrumentation](https://github.com/open-telemetry/opentelemetry-dotnet-instrumentation) which automatically instruments:
...
The Dockerfile downloads and configures the automatic instrumentation agent using environment variables:
...
```bash
kubectl scale deployment dotnet-aspnetcore-k8s-auto --replicas=3
```
...
kubectl rollout restart deployment/dotnet-aspnetcore-k8s-auto
```
...
1. **Check pod status**:
   ```bash
   kubectl describe pod -l app=dotnet-aspnetcore-k8s-auto
   ```
...
3. **Check image availability in Minikube**:
   ```bash
   minikube image ls | grep dotnet-aspnetcore-k8s-auto
   ```
...
spec:
  podSelector:
    matchLabels:
      app: dotnet-aspnetcore-k8s-auto
  egress:
  - to:
    - namespaceSelector:
        matchLabels:
